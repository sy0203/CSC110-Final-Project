"""
CSC110 Fall 2021 Final Project: Calculation Helper
...
def percentage_list(indexes: list[float]) -> list[float]:
    """
    Returns a list of percentage change across the data
...
def percentage_list_index(indexes: list[float]) -> list[float]:
    """
    Returns a list of percentage change across the data such that the
    list is parallel to the covid cases.
    """
    start = indexes[56]
    percent_changes = []
    for x in range(57, len(indexes)):
        percent_changes.append(percentage_change(start, indexes[x]))

    return percent_changes


def correlation_coeff(p: list, q: list) -> float:
    """
    ...
    x = [p[i] for i in range(len(q))]
    y = [q[i] for i in range(len(x))]
    n = len(y)
...
class Country:
    ...
    A country, described by its financial state and covid cases.
        Instance Attributes:
            ...
            - percent_changes: The percentage change in stock index prices every week
        ...
        >>> percent_changes = percentage_list(prices)
        >>> mean, median, stan_dev = get_stats(percent_changes)
        >>> r2 = correlation_coeff(cases, prices)
        >>> rec = index_recovered(prices[0], prices[len(prices) - 1])
        >>> usa = Country('United States', dates, cases, cdates, prices, percent_changes, mean, median, stan_dev, r2, rec)
        """
    name: str
    dates: list[str]
    cases: list
