...
class Screen:
    """
    The screen class represents a page that consists of buttons and behaviours.
    """

    def __init__(self, title: str, buttons: list, signal: str, operation: tuple):
        """
        Initialize a new Screen, by assigning paramenter values to their respective instance variables.
        """
        self.title = title
    ...
    def display_page(self) -> None:
        ...
        # Adding a title to the page. Since the menu is unique, there is a separate condition for it.
        if self.title == 'Main Menu':
            self.draw_text("Covid Research", pygame.font.Font(None, 90), (22, 25, 26), screen, 60)
            self.draw_text('Selina Abhijit Phadiya, Natasha Sharan, Seyoung Yoo', pygame.font.Font(None, 30),
                           (22, 25, 26), screen, 110)
        else:
    ...
    def update_function(self, operation: tuple) -> None:
        """This updates the description function if it is available."""
        self.operation, self.input = (0, 0)
        if len(operation) == 2:
            self.operation, self.input = operation
...
class Button:
    """
    This class represents an interactive element of the program that sends signals upon interaction.
    """
    ...
    def change_text(self, new_text: str) -> None:
        """This is a helper function for rendering the display"""
    ...
    def check_click(self) -> None:
        """This function monitors how the mouse interacts with the button."""
...
def about_screen(_: int) -> None:
    """Custom display for the about screen."""
    pos = 120
    size = 33
    ...
    about.draw_text('Selina Abhijit Phadiya, Natasha Sharan, Seyoung Yoo', pygame.font.Font(None, size),
                    (22, 25, 26), screen, pos + size * 11)


def option_screen(_: int) -> None:
    """Custom display for the options screen."""
...
def choro_stock_graph_description(_: int) -> None:
    """Custom display for the choropleth world map screen."""
    ...
    for x in range(5):
        ...
        about.draw_text_x(str(cs[x].prices[56]), pygame.font.Font(None, 30), (22, 25, 26), screen, 163, rows_y + 30 * x)
...
def all_line_graphs(_: int) -> None:
    """Custom display for the all_graphs screen."""
    cs = list(countries_list.values())
    char_y = 330

    about.draw_text("This graph displays the benchmark index funds for all five countries over time.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 100)
    about.draw_text("The time interval spans from pre-covid in 2019 to a more recent date ",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 130)
    about.draw_text("in the pandemic. Many countries experienced a stock market crash, ",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 160)
    about.draw_text("however the five we are focusing on in this study, have recovered",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 190)
    about.draw_text("and exceeded their pre-covid prices. Some countries recovered faster ",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 220)
    about.draw_text("than others, and some performed better than others as can be evidenced ",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 250)
    about.draw_text("by the following information",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 280)

    about.draw_text_x("Country", pygame.font.Font(None, 30), (22, 25, 26), screen, 100, char_y)
    about.draw_text_x("# of months", pygame.font.Font(None, 30), (22, 25, 26), screen, 300, char_y)
    about.draw_text_x("Price Increase (%)", pygame.font.Font(None, 30), (22, 25, 26), screen, 500, char_y)

    rows_y = char_y + 50
    for x in range(5):
        about.draw_text_x(cs[x].name, pygame.font.Font(None, 30), (22, 25, 26), screen, 100, rows_y + 30 * x)
        about.draw_text_x(str(calc.count_months(cs[x].dates, cs[x].prices)),
                          pygame.font.Font(None, 30), (22, 25, 26), screen, 300, rows_y + 30 * x)
        about.draw_text_x(str(calc.percentage_change(cs[x].prices[56], cs[x].prices[-1])),
                          pygame.font.Font(None, 30), (22, 25, 26), screen, 500,
                          rows_y + 30 * x)


def table_screen(_: int) -> None:
    """Custom display for the table screen."""
    pos = 150
    about.draw_text("This table displays the percentage change in stock prices for each country",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, pos)
    about.draw_text("after specific intervals of time. From here it can be observed that the",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, pos + 30)
    about.draw_text("countries experienced changes in their prices at different rates, some increasing",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, pos + 30 * 2)
    about.draw_text("faster than others. Overall it can be concluded that the country that experienced",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, pos + 30 * 3)
    about.draw_text("the highest percentage increase in price is Brazil, and the lowest is India.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, pos + 30 * 4)


def line_graph_single(country: Country) -> None:
    """Custom display for any country on the line graph screen."""

    pos = 200
    about.draw_text("This graph will display the change in stock prices and covid cases over time.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 120)

    about.draw_text_x("Mean", pygame.font.Font(None, 45), (22, 25, 26), screen, 50, pos)
    about.draw_text_x(str(country.mean), pygame.font.Font(None, 45), (22, 25, 26), screen, 400, pos)

    about.draw_text_x("Median", pygame.font.Font(None, 45), (22, 25, 26), screen, 50, pos + 60)
    about.draw_text_x(str(country.median), pygame.font.Font(None, 45), (22, 25, 26), screen, 400, pos + 60)

    about.draw_text_x("Standard Deviation", pygame.font.Font(None, 45), (22, 25, 26), screen, 50, pos + 60 * 2)
    about.draw_text_x(str(country.standard_deviation),
                      pygame.font.Font(None, 45), (22, 25, 26), screen, 400, pos + 60 * 2)

    about.draw_text("Mean and median are measures of the average of percentage changes.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 400)
    about.draw_text("The standard deviation is a measure of how spread out the graph is.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 430)
    about.draw_text("By the standard normal distribution, it can be said that 68% of ",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 460)
    about.draw_text("the graph will be within +/- 1 standard deviation.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 490)


def sp_description(country: Country) -> None:
    """Custom display for any country on the scatterplot graph screen."""

    about.draw_text("This graph plots the country's covid cases against its stock index funds.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 130)
    about.draw_text("A general interpretation of the relation between these variables can be",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 160)
    about.draw_text("given by the correlation coefficient which is " + str(country.correlation_coefficient),
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 190)
    about.draw_text("The closer this value is to either 1 or -1, the more direct the relationship.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 220)
    about.draw_text("The closer the value is to zero, the weaker the relationship.",
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 250)
    value = '0'
    level = 'weaker'
    if 1 - abs(country.correlation_coefficient) < abs(country.correlation_coefficient):
        value = '1'
        level = 'stronger'
    about.draw_text("In this case, the correlation coefficient is closer to " + value + ',',
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 290)

    about.draw_text("Thus, the relation between covid cases and stock index prices is " + level,
                    pygame.font.Font(None, 30), (22, 25, 26), screen, 320)
    ...
    if status[0] == 'begin':
        while status[0] != 'back':
            ...
            if status[0] == 'choro ig':
                graph_manager.choropleth_graph(list(countries_list.values()))
                ...
                while status[0] != 'back' and status[0] != 'begin':
                    ...
                    if status[0] in stats_titles:
                        ...
                        country_page.update_function((sp_description, countries_list[stats_titles[status[0]]]))
                        state = status[0]
                        start_page(country_page, state, country_graphs_scatter)
